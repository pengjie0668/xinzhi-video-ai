...
class VLM:
    def __init__(self,
        ...
        self._local_proxy = local_proxy or Config.LOCAL_PROXY
    ...
    def query(self,
        ...
        if is_gemini:
            ...
            return self._get_gemini_client().chat(text=prompt, images=processed_images, model=model)
        elif is_gpt:
            return self._get_gpt_client().chat(text=prompt, images=image_paths or [], model=model)
        else:
            ...
            for p in image_paths or []:
                if p.startswith("data:"):
                    ...
                    file_urls.append(f"file://{abs_path}")
            return self._get_dashscope_client().chat(text=prompt, images=file_urls, model=model, stream=False)
