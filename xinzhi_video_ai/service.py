...
"""
Xinzhi Video AI Core - Service Layer
...
from xinzhi_video_ai.services.api_media import APIProviderMediaService
from xinzhi_video_ai.services.image_analysis import ImageAnalysisService
from xinzhi_video_ai.services.video_analysis import VideoAnalysisService
from xinzhi_video_ai.services.api_asset_analysis import APIAssetAnalysisService
...
from xinzhi_video_ai.pipelines.standard import StandardPipeline
from xinzhi_video_ai.pipelines.custom import CustomPipeline
from xinzhi_video_ai.pipelines.asset_based import AssetBasedPipeline


class XinzhiVideoAICore:
    """
    Xinzhi Video AI Core - Service Layer
    ...
    Usage:
        ...
        media = await xinzhi_video_ai.media(prompt="a cat")
        
        # Check active capabilities
        print(f"Using LLM: {xinzhi_video_ai.llm.active}")
        print(f"Available TTS: {xinzhi_video_ai.tts.available}")
    
    Architecture (Simplified):
        XinzhiVideoAICore (this class)
    ...
    def __init__(self, config_path: str = "config.yaml"):
        """
        Initialize Xinzhi Video AI Core
    ...
    async def initialize(self):
        ...
        if self._initialized:
            logger.warning("Xinzhi Video AI already initialized")
            return
        
        logger.info("🚀 Initializing Xinzhi Video AI...")
        ...
        logger.info("✅ Xinzhi Video AI initialized successfully\n")
    ...
    def project_name(self) -> str:
        ...
        return self.config.get("project_name", "芯智未来 AI 视频引擎")
    
    def __repr__(self) -> str:
        """String representation"""
        status = "initialized" if self._initialized else "not initialized"
        pipelines = f"pipelines={list(self.pipelines.keys())}" if self._initialized else ""
        return f"<XinzhiVideoAICore project={self.project_name!r} status={status} {pipelines}>"
