...
from xinzhi_video_ai.pipelines.linear import LinearVideoPipeline, PipelineContext
...
class AssetBasedPipeline(LinearVideoPipeline):
    ...
    def __init__(self, core):
        ...
        Args:
            core: XinzhiVideoAICore instance
        """
        super().__init__(core)
    ...
    async def __call__(
        ...
        **kwargs
        ...
        """
        from xinzhi_video_ai.pipelines.linear import PipelineContext
        ...
        for i, asset_path in enumerate(assets, 1):
            ...
            if asset_type == "image":
                ...
                try:
                    ...
                    logger.warning(f"Image analysis failed for {asset_path_obj.name}: {e}, using fallback")
                    user_text = str(context.input_text or "").strip()
                    description = (
                        f"Image asset related to the user prompt: {user_text}"
                        if user_text
                        else "Image asset (analysis failed)"
                    )
    ...
    async def generate_content(self, context: PipelineContext) -> PipelineContext:
        ...
        """
        from xinzhi_video_ai.prompts.asset_script_generation import build_asset_script_prompt
