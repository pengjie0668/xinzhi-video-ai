...
from web.state.session import init_session_state, init_i18n, get_xinzhi_video_ai
...
def render_sidebar_controls(xinzhi_video_ai):
    """Render sidebar with statistics and filters"""
    with st.sidebar:
        # Statistics
        st.markdown(f"**📊 {tr('history.total_tasks')}**")
        stats = run_async(xinzhi_video_ai.history.get_statistics())
...
def render_grid_task_card(task: dict, xinzhi_video_ai):
    ...
    # Get input text
    detail = run_async(xinzhi_video_ai.history.get_task_detail(task_id))
    input_text = ""
        ...
        if st.session_state.get(f"confirm_delete_{task_id}", False):
            ...
            with col1:
                if st.button("✅", key=f"confirm_yes_{task_id}", use_container_width=True):
                    try:
                        success = run_async(xinzhi_video_ai.history.delete_task(task_id))
                        if success:
...
def render_task_detail_modal(task_id: str, xinzhi_video_ai):
...
def main():
    ...
    xinzhi_video_ai = get_xinzhi_video_ai()
    
    # Sidebar: Statistics + Filters
    filter_status, sort_by, sort_order, page_size = render_sidebar_controls(xinzhi_video_ai)
    ...
    if show_detail_for:
        render_task_detail_modal(show_detail_for, xinzhi_video_ai)
        return
    
    # Otherwise, show the grid list
    # Get task list
    result = run_async(xinzhi_video_ai.history.get_task_list(
        ...
        for i in range(0, len(tasks), CARDS_PER_ROW):
            ...
            for j in range(CARDS_PER_ROW):
                ...
                if task_idx < len(tasks):
                    with cols[j]:
                        render_grid_task_card(tasks[task_idx], xinzhi_video_ai)
