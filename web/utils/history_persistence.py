...
async def save_web_generation_history(
    ...
    *,
    ...
    if not getattr(xinzhi_video_ai, "persistence", None):
        logger.warning("Xinzhi Video AI persistence service is not initialized; skipping history save")
        return
    ...
    metadata = {
        ...
        "config": {
            "llm_model": xinzhi_video_ai.config.get("llm", {}).get("model", "unknown"),
            "llm_base_url": xinzhi_video_ai.config.get("llm", {}).get("base_url", "unknown"),
            "source": "web",
        },
    }

    await xinzhi_video_ai.persistence.save_task_metadata(task_id, metadata)
