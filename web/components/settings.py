    ...
    with st.expander(tr("settings.title"), expanded=not is_configured):
        ...
        with llm_col:
            with st.container(border=True):
                ...
                from xinzhi_video_ai.llm_presets import get_preset_names, get_preset, find_preset_by_base_url_and_model
                ...
                if load_clicked:
                    if llm_api_key and llm_base_url:
                        try:
                            from xinzhi_video_ai.utils.llm_util import fetch_available_models
                ...
                if test_clicked:
                    if llm_api_key and llm_base_url:
                        try:
                            from xinzhi_video_ai.utils.llm_util import test_llm_connection
                            with st.spinner(tr("settings.llm.loading_models")):
        ...
        with col2:
            if st.button(tr("btn.reset_config"), use_container_width=True, key="reset_config_btn"):
                # Reset to default
                from xinzhi_video_ai.config.schema import XinzhiVideoAIConfig
                config_manager.config = XinzhiVideoAIConfig()
                config_manager.save()
