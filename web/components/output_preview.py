...
def render_output_preview(xinzhi_video_ai, video_params):
    ...
    if is_batch:
        ...
        render_batch_output(xinzhi_video_ai, video_params)
    else:
        # Single video generation mode (original logic)
        render_single_output(xinzhi_video_ai, video_params)


def render_single_output(xinzhi_video_ai, video_params):
    ...
    with st.container(border=True):
        ...
        if st.button(tr("btn.generate"), type="primary", use_container_width=True):
            ...
            try:
                ...
                result = run_async(xinzhi_video_ai.generate_video(**video_params))
...
def render_batch_output(xinzhi_video_ai, video_params):
    ...
    with st.container(border=True):
        ...
        if st.button(
            ...
            batch_result = batch_manager.execute_batch(
                xinzhi_video_ai=xinzhi_video_ai,
                topics=topics,
